...
summarize_and_plot <- function(data, x_var, y_vars, plot_types,
  ...
  create_plot <- function(y_var, plot_type) {
    p <- switch(plot_type,
                "box" = ggplot(data, aes(x = x_var, y = y_var, color = shQuote(y_var))) +
                  geom_boxplot() +
                  scale_color_manual(values = y_var_colors) +
                  labs(title = paste("Boxplot of", y_var, "by", x_var)),

                "scatter" = ggplot(data, aes(x = x_var, y = y_var, color = shQuote(y_var))) +
                  geom_point() +
                  scale_color_manual(values = y_var_colors) +
                  labs(title = paste("Scatter plot of", y_var, "by", x_var)),

                "density" = ggplot(data, aes(x = y_var, color = shQuote(y_var))) +
                  geom_density() +
                  scale_color_manual(values = y_var_colors) +
                  labs(title = paste("Density plot of", y_var)),

                "histogram" = ggplot(data, aes(x = y_var, fill = shQuote(y_var))) +
                  geom_histogram(bins = 30, color = "black") +
                  scale_fill_manual(values = y_var_colors) +
                  labs(title = paste("Histogram of", y_var)),

                "bar" = ggplot(data, aes(x = x_var, y = y_var, fill = shQuote(y_var))) +
  ...
  for (plot_type in plot_types) {
    if (combined_plot) {
      ...
      for (y_var in y_vars) {
        p <- switch(plot_type,
                    "box" = p + geom_boxplot(aes(y = y_var, fill = shQuote(y_var)),
                                             color = y_var_colors[y_var], alpha = 0.5),

                    "scatter" = p + geom_point(aes(y = y_var, color = shQuote(y_var))) +
                      scale_color_manual(values = y_var_colors),

                    "density" = p + geom_density(aes(x = y_var, color = shQuote(y_var)),
                                                 alpha = 0.5) +
                      scale_color_manual(values = y_var_colors),

                    "histogram" = p + geom_histogram(aes(x = y_var, fill = shQuote(y_var)),
                                                     bins = 30, color = "black", alpha = 0.5, position = "identity") +
                      scale_fill_manual(values = y_var_colors),

                    "bar" = p + geom_bar(aes(y = y_var, fill = shQuote(y_var)),
