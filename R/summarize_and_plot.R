...
summarize_and_plot <- function(data, x_var, y_vars, plot_types,
  ...
  # Check that x_var exists in the dataset
  if (!x_var %in% names(data)) {
    stop(paste("The x variable", x_var, "does not exist in the dataset."))
  }

  # Check that all y_vars exist in the dataset
  missing_y_vars <- y_vars[!y_vars %in% names(data)]
  if (length(missing_y_vars) > 0) {
    stop(paste("The following y variables do not exist in the dataset:", paste(missing_y_vars, collapse = ", ")))
  }
  # columns are numeric for summaries
  data_numeric <- data %>%
  ...
  if (is.null(y_var_colors)) {
    y_var_colors <- stats::setNames(grDevices::rainbow(length(y_vars)), y_vars)
  }
  ...
  create_plot <- function(y_var, plot_type) {
    p <- switch(plot_type,
                "box" = ggplot(data, aes_string(x = x_var, y = y_var, color = shQuote(y_var))) +
                  geom_boxplot() +
                  scale_color_manual(values = y_var_colors) +
                  labs(title = paste("Boxplot of", y_var, "by", x_var)),

                "scatter" = ggplot(data, aes_string(x = x_var, y = y_var, color = shQuote(y_var))) +
                  geom_point() +
                  scale_color_manual(values = y_var_colors) +
                  labs(title = paste("Scatter plot of", y_var, "by", x_var)),

                "density" = ggplot(data, aes_string(x = y_var, color = shQuote(y_var))) +
                  geom_density() +
                  scale_color_manual(values = y_var_colors) +
                  labs(title = paste("Density plot of", y_var)),

                "histogram" = ggplot(data, aes_string(x = y_var, fill = shQuote(y_var))) +
                  geom_histogram(bins = 30, color = "black") +
                  scale_fill_manual(values = y_var_colors) +
                  labs(title = paste("Histogram of", y_var)),

                "bar" = ggplot(data, aes_string(x = x_var, y = y_var, fill = shQuote(y_var))) +
  ...
  # Individual/combined plots based on argument
  for (plot_type in plot_types) {
    if (combined_plot) {
      # Combined plot with all y_vars in a single plot
      p <- ggplot(data, aes_string(x = x_var)) +
        labs(title = paste("Combined", plot_type, "of", paste(y_vars, collapse = ", "), "by", x_var))

      for (y_var in y_vars) {
        p <- switch(plot_type,
                    "box" = p + geom_boxplot(aes_string(y = y_var, fill = shQuote(y_var)),
                                             color = y_var_colors[y_var], alpha = 0.5),

                    "scatter" = p + geom_point(aes_string(y = y_var, color = shQuote(y_var))) +
                      scale_color_manual(values = y_var_colors),

                    "density" = p + geom_density(aes_string(x = y_var, color = shQuote(y_var)),
                                                 alpha = 0.5) +
                      scale_color_manual(values = y_var_colors),

                    "histogram" = p + geom_histogram(aes_string(x = y_var, fill = shQuote(y_var)),
                                                     bins = 30, color = "black", alpha = 0.5, position = "identity") +
                      scale_fill_manual(values = y_var_colors),

                    "bar" = p + geom_bar(aes_string(y = y_var, fill = shQuote(y_var)),
  ...
  # Display all plots
  wrap_plots(plots, ncol = 2)  # Adjust as needed
